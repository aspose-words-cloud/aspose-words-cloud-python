...
class Bookmark(object):
	...
	def get_document_bookmark_by_name(self):
		...
		if(file_upload_response["Code"] == 200 and file_upload_response["Status"] == "OK"):
			request = asposewordscloud.models.requests.GetDocumentBookmarkByNameRequest(filename, bookmark_name)
			result = self.words_api.get_document_bookmark_by_name(request)
			print("Result {}".format(result))

bookmark = Bookmark()
bookmark.get_document_bookmark_by_name()