...
class Table(object):
	...
	def update_section_page_setup(self):
		...
		if(file_upload_response["Code"] == 200 and file_upload_response["Status"] == "OK"):
			request = asposewordscloud.models.requests.UpdateSectionPageSetupRequest(filename, index, body)
			result = self.words_api.update_section_page_setup(request)
			print("Result {}".format(result))

table = Table()
table.update_section_page_setup()
