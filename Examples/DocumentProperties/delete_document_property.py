...
class Document(object):
	...
	def delete_document_property(self):
		filename = 'test_multi_pages.docx'
		dest_name = 'DeleteDocumentProperty.docx'
		...
		if(file_upload_response["Code"] == 200 and file_upload_response["Status"] == "OK"):
			request = asposewordscloud.models.requests.DeleteDocumentPropertyRequest(filename, property_name, dest_file_name=dest_name)
			result = self.words_api.delete_document_property(request)
			print("Result {}".format(result))

document = Document()
document.delete_document_property()