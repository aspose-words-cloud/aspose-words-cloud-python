...
class Document(object):
	...
	def get_document_property(self):
		...
		if(file_upload_response["Code"] == 200 and file_upload_response["Status"] == "OK"):
			request = asposewordscloud.models.requests.GetDocumentPropertyRequest(filename, property_name)
			result = self.words_api.get_document_property(request)
			print("Result {}".format(result))

document = Document()
document.get_document_property()