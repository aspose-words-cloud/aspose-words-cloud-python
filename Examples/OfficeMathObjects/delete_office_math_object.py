...
class Document(object):
	...
	def __init__(self):
		...
		self.dataFolder = os.path.join(ABSPATH, "TestData/DocumentElements/MathObjects")

	def delete_office_math_object(self):
		...
		if(file_upload_response["Code"] == 200 and file_upload_response["Status"] == "OK"):
			request = asposewordscloud.models.requests.DeleteOfficeMathObjectRequest(filename, index)
			result = self.words_api.delete_office_math_object(request)
			print("Result {}".format(result))

document = Document()
document.delete_office_math_object()