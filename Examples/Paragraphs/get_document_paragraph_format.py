...
class Document(object):
	...
	def get_document_paragraph_format(self):
		...
		if(file_upload_response["Code"] == 200 and file_upload_response["Status"] == "OK"):
			request = asposewordscloud.models.requests.GetDocumentParagraphFormatRequest(filename, index)
			result = self.words_api.get_document_paragraph_format(request)
			print("Result {}".format(result))

document = Document()
document.get_document_paragraph_format()
