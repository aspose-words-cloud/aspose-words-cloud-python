...
class Document(object):
	...
	def post_document_paragraph_format(self):
		...
		if(file_upload_response["Code"] == 200 and file_upload_response["Status"] == "OK"):
			request = asposewordscloud.models.requests.PostDocumentParagraphFormatRequest(filename, body, '', index)
			result = self.words_api.post_document_paragraph_format(request)
			print("Result {}".format(result))

document = Document()
document.post_document_paragraph_format()
