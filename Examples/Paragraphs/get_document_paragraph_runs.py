...
class Document(object):
	...
	def get_document_paragraph_runs(self):
		...
		if(file_upload_response["Code"] == 200 and file_upload_response["Status"] == "OK"):
			request = asposewordscloud.models.requests.GetDocumentParagraphRunsRequest(filename, paragraph_path)
			result = self.words_api.get_document_paragraph_runs(request)
			print("Result {}".format(result))

document = Document()
document.get_document_paragraph_runs()
