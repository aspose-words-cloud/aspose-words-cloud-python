...
class Document(object):
	...
	def put_form_field(self):
		...
		body = asposewordscloud.FormFieldTextInput(name='FullName', enabled=True, calculate_on_exit=True, status_text='',
                                                 text_input_type='Regular', text_input_default='123', text_input_format='UPPERCASE')
		...
		if(file_upload_response["Code"] == 200 and file_upload_response["Status"] == "OK"):
			request = asposewordscloud.models.requests.PutFormFieldRequest(filename, body, dest_file_name=dest_name, node_path='sections/0')
			result = self.words_api.put_form_field(request)
			print("Result {}".format(result))

document = Document()
document.put_form_field()