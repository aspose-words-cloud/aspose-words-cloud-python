...
class Document(object):
	...
	def post_update_document_fields(self):
		...
		if(file_upload_response["Code"] == 200 and file_upload_response["Status"] == "OK"):
			request = asposewordscloud.models.requests.PostUpdateDocumentFieldsRequest(filename)
			result = self.words_api.post_update_document_fields(request)
			print("Result {}".format(result))

document = Document()
document.post_update_document_fields()