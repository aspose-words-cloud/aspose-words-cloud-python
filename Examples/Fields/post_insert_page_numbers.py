...
class Document(object):
	...
	def post_insert_page_numbers(self):
		...
		if(file_upload_response["Code"] == 200 and file_upload_response["Status"] == "OK"):
			request = asposewordscloud.models.requests.PostInsertPageNumbersRequest(filename, body, dest_file_name=dest_name)
			result = self.words_api.post_insert_page_numbers(request)
			print("Result {}".format(result))

document = Document()
document.post_insert_page_numbers()