...
class Document(object):
	...
	def post_document_paragraph_run_font(self):
		...
		if(file_upload_response["Code"] == 200 and file_upload_response["Status"] == "OK"):
			request = asposewordscloud.models.requests.PostDocumentParagraphRunFontRequest(filename, body, paragraph_path, index, dest_file_name=dest_name)
			result = self.words_api.post_document_paragraph_run_font(request)
			print("Result {}".format(result))

document = Document()
document.post_document_paragraph_run_font()
