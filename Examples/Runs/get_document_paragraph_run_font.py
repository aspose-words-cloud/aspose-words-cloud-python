...
class Document(object):
	...
	def get_document_paragraph_run_font(self):
		...
		if(file_upload_response["Code"] == 200 and file_upload_response["Status"] == "OK"):
			request = asposewordscloud.models.requests.GetDocumentParagraphRunFontRequest(filename, paragraph_path, index)
			result = self.words_api.get_document_paragraph_run_font(request)
			print("Result {}".format(result))

document = Document()
document.get_document_paragraph_run_font()
