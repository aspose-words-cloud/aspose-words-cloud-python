...
class Document(object):
	...
	def __init__(self):
		...
		self.dataFolder = os.path.join(ABSPATH, "TestData/DocumentActions/DocumentProtection")

	def delete_unprotect_document(self):
		...
		if(file_upload_response["Code"] == 200 and file_upload_response["Status"] == "OK"):
			request = asposewordscloud.models.requests.DeleteUnprotectDocumentRequest(filename, body)
			result = self.words_api.delete_unprotect_document(request)
			print("Result {}".format(result))

document = Document()
document.delete_unprotect_document()
