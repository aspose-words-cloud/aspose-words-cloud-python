...
class Document(object):
	...
	def post_change_document_protection(self):
		...
		if(file_upload_response["Code"] == 200 and file_upload_response["Status"] == "OK"):
			request = asposewordscloud.models.requests.PostChangeDocumentProtectionRequest(filename, body)
			result = self.words_api.post_change_document_protection(request)
			print("Result {}".format(result))

document = Document()
document.post_change_document_protection()
