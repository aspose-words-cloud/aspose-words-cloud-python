...
class Document(object):
	...
	def get_document_hyperlink_by_index(self):
		...
		if(file_upload_response["Code"] == 200 and file_upload_response["Status"] == "OK"):
			request = asposewordscloud.models.requests.GetDocumentHyperlinkByIndexRequest(filename, index)
			result = self.words_api.get_document_hyperlink_by_index(request)
			print("Result {}".format(result))

document = Document()
document.get_document_hyperlink_by_index()