...
class Table(object):
	...
	def post_replace_text(self):
		...
		if(file_upload_response["Code"] == 200 and file_upload_response["Status"] == "OK"):
			request = asposewordscloud.models.requests.PostReplaceTextRequest(filename, body, dest_file_name=dest_name)
			result = self.words_api.post_replace_text(request)
			print("Result {}".format(result))

table = Table()
table.post_replace_text()
