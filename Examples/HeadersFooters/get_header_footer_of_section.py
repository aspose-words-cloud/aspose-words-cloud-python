...
class Document(object):
	...
	def __init__(self):
		...
		self.dataFolder = os.path.join(ABSPATH, "TestData/DocumentElements/HeaderFooters")

	def get_header_footer_of_section(self):
		...
		if(file_upload_response["Code"] == 200 and file_upload_response["Status"] == "OK"):
			request = asposewordscloud.models.requests.GetHeaderFooterOfSectionRequest(filename, index, section_index)
			result = self.words_api.get_header_footer_of_section(request)
			print("Result {}".format(result))

document = Document()
document.get_header_footer_of_section()