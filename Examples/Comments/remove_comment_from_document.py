...
class Document(object):
	...
	def remove_comment_from_document(self):
		...
		if(file_upload_response["Code"] == 200 and file_upload_response["Status"] == "OK"):
			request = asposewordscloud.models.requests.DeleteCommentRequest(filename, comment_index)
			result = self.words_api.delete_comment(request)
			print("Result {}".format(result))

document = Document()
document.remove_comment_from_document()