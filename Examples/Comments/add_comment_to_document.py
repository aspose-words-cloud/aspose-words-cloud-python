...
class Document(object):
	...
	def add_comment_to_document(self):
		filename = 'test_multi_pages.docx'
		node_link = asposewordscloud.NodeLink(None, '0.0.3')
		doc_pos = asposewordscloud.DocumentPosition(node_link, 0)
		comment_body = asposewordscloud.Comment(None, 'Yaroslav Ekimov', initial='YE', range_start=doc_pos, range_end=doc_pos, text='A new comment')
		...
		if(file_upload_response["Code"] == 200 and file_upload_response["Status"] == "OK"):
			request = asposewordscloud.models.requests.PutCommentRequest(filename, comment_body)
			result = self.words_api.put_comment(request)
			print("Result {}".format(result))

document = Document()
document.add_comment_to_document()