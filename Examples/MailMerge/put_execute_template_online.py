...
class Document(object):
	...
	def put_execute_template_online(self):
		filename = 'SampleExecuteTemplate.docx'

		filePath = os.path.join(self.dataFolder, filename)
		templateDataFilePath = os.path.join(self.dataFolder, 'SampleExecuteTemplateData.txt')
		
		request = asposewordscloud.models.requests.PutExecuteTemplateOnlineRequest(filePath, templateDataFilePath)
		result = self.words_api.put_execute_template_online(request)
		print("Result {}".format(result))

document = Document()
document.put_execute_template_online()