...
class Document(object):
	...
	def post_document_execute_mail_merge(self):
		...
		if(file_upload_response["Code"] == 200 and file_upload_response["Status"] == "OK"):
			request = asposewordscloud.models.requests.PostDocumentExecuteMailMergeRequest(filename, templateData, dest_file_name=dest_name)
			result = self.words_api.post_document_execute_mail_merge(request)
			print("Result {}".format(result))

document = Document()
document.post_document_execute_mail_merge()