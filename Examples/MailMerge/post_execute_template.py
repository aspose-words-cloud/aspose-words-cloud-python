...
class Document(object):
	...
	def post_execute_template(self):
		#filename = 'TestExecuteTemplate.doc'
		filename = 'DefectListtemplateen.docx'
		dest_name = 'SentPostExecuteTemplate.docx'
		...
		#templateDataFilePath = os.path.join(self.dataFolder, 'TestExecuteTemplateData.txt')
		templateDataFilePath = os.path.join(self.dataFolder, 'sent.txt')
		with open(templateDataFilePath, 'rb') as f:
			templateData = f.read()

		# Check if file uploaded successfully to Cloud Storage
		if(file_upload_response["Code"] == 200 and file_upload_response["Status"] == "OK"):
			request = asposewordscloud.models.requests.PostExecuteTemplateRequest(filename, templateData, dest_file_name=dest_name)
			result = self.words_api.post_execute_template(request)
			print("Result {}".format(result))

document = Document()
document.post_execute_template()