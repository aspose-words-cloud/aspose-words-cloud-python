...
class Document(object):
	...
	def __init__(self):
		...
		self.dataFolder = os.path.join(ABSPATH, "TestData/DocumentElements/Footnotes")

	def post_footnote(self):
		...
		if(file_upload_response["Code"] == 200 and file_upload_response["Status"] == "OK"):
			request = asposewordscloud.models.requests.PostFootnoteRequest(filename, footnote, index)
			result = self.words_api.post_footnote(request)
			print("Result {}".format(result))

document = Document()
document.post_footnote()