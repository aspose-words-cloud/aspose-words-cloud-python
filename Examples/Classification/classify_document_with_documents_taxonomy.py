...
class Classification(object):
	...
	# Classify document with "documents" taxonomy
	def classify_document_with_documents_taxonomy(self):
		...
		if(file_upload_response["Code"] == 200 and file_upload_response["Status"] == "OK"):
			request = asposewordscloud.models.requests.ClassifyDocumentRequest(filename, taxonomy=taxonomy)
			result = self.words_api.classify_document(request)
			print("Result {}".format(result))

classification = Classification()
classification.classify_document_with_documents_taxonomy()