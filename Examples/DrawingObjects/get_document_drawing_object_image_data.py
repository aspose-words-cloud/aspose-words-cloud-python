...
class Document(object):
	...
	def get_document_drawing_object_image_data(self):
		...
		if(file_upload_response["Code"] == 200 and file_upload_response["Status"] == "OK"):
			request = asposewordscloud.models.requests.GetDocumentDrawingObjectImageDataRequest(filename, index, node_path='sections/0')
			result = self.words_api.get_document_drawing_object_image_data(request)
			print("Result {}".format(result))

document = Document()
document.get_document_drawing_object_image_data()