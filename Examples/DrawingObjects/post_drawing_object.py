...
class Document(object):
	...
	def post_drawing_object(self):
		filename = 'test_multi_pages.docx'
		data = os.path.join(self.dataFolder, 'aspose-cloud.png')
		index = 0
		...
		if(file_upload_response["Code"] == 200 and file_upload_response["Status"] == "OK"):
			request = asposewordscloud.models.requests.PostDrawingObjectRequest(filename, '{"Left": 0}', data, index)
			result = self.words_api.post_drawing_object(request)
			print("Result {}".format(result))

document = Document()
document.post_drawing_object()