...
class Document(object):
	...
	def render_drawing_object(self):
		...
		if(file_upload_response["Code"] == 200 and file_upload_response["Status"] == "OK"):
			request = asposewordscloud.models.requests.RenderDrawingObjectRequest(filename, _format, index, node_path='sections/0')
			result = self.words_api.render_drawing_object(request)
			print("Result {}".format(result))

document = Document()
document.render_drawing_object()