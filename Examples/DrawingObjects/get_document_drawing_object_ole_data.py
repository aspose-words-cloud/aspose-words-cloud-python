...
class Document(object):
	...
	def __init__(self):
		...
		self.dataFolder = os.path.join(ABSPATH, "TestData/DocumentElements/DrawingObjects")

	def get_document_drawing_object_ole_data(self):
		...
		if(file_upload_response["Code"] == 200 and file_upload_response["Status"] == "OK"):
			request = asposewordscloud.models.requests.GetDocumentDrawingObjectOleDataRequest(filename, index, node_path='sections/0')
			result = self.words_api.get_document_drawing_object_ole_data(request)
			print("Result {}".format(result))

document = Document()
document.get_document_drawing_object_ole_data()