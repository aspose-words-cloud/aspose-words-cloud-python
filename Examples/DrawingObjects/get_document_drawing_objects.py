...
class Document(object):
	...
	def get_document_drawing_objects(self):
		...
		if(file_upload_response["Code"] == 200 and file_upload_response["Status"] == "OK"):
			request = asposewordscloud.models.requests.GetDocumentDrawingObjectsRequest(filename)
			result = self.words_api.get_document_drawing_objects(request)
			print("Result {}".format(result))

document = Document()
document.get_document_drawing_objects()