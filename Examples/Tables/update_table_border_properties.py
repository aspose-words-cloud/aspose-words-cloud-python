...
class Table(object):
	...
	def update_table_border_properties(self):
		...
		node_path = 'tables/1/rows/0/cells/0/'
		index = 0
		border_properties = asposewordscloud.Border(None, 'Left', asposewordscloud.XmlColor(None, 2), 6, 'DashDotStroker', 2, True)
		...
		if(file_upload_response["Code"] == 200 and file_upload_response["Status"] == "OK"):
			request = asposewordscloud.models.requests.UpdateBorderRequest(filename, border_properties, node_path, index)
			result = self.words_api.update_border(request)
			print("Result {}".format(result))

table = Table()
table.update_table_border_properties()