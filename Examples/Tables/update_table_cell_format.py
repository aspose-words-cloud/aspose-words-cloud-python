...
class Table(object):
	...
	def update_table_cell_format(self):
		...
		if(file_upload_response["Code"] == 200 and file_upload_response["Status"] == "OK"):
			request = asposewordscloud.models.requests.UpdateTableCellFormatRequest(filename, source_path, index, format=body)
			result = self.words_api.update_table_cell_format(request)
			print("Result {}".format(result))

table = Table()
table.update_table_cell_format()