...
class Table(object):
	...
	def reset_table_borders_properties_to_default_values(self):
		...
		if(file_upload_response["Code"] == 200 and file_upload_response["Status"] == "OK"):
			request = asposewordscloud.models.requests.DeleteBordersRequest(filename, node_path)
			result = self.words_api.delete_borders(request)
			print("Result {}".format(result))

table = Table()
table.reset_table_borders_properties_to_default_values()