...
class Table(object):
	...
	def reset_table_border_properties_to_default_values(self):
		filename = 'TablesGet.docx'
		node_path = 'tables/1/rows/0/cells/0/'
		index = 0
		dest_name = 'DeleteTableBorder.docx'
		...
		if(file_upload_response["Code"] == 200 and file_upload_response["Status"] == "OK"):
			request = asposewordscloud.models.requests.DeleteBorderRequest(filename, node_path, index,
                                                                     			dest_file_name=dest_name)
			result = self.words_api.delete_border(request)
			print("Result {}".format(result))

table = Table()
table.reset_table_border_properties_to_default_values()