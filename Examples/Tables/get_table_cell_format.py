...
class Table(object):
	...
	def get_table_cell_format(self):
		...
		if(file_upload_response["Code"] == 200 and file_upload_response["Status"] == "OK"):
			request = asposewordscloud.models.requests.GetTableCellFormatRequest(filename, source_path, index)
			result = self.words_api.get_table_cell_format(request)
			print("Result {}".format(result))

table = Table()
table.get_table_cell_format()